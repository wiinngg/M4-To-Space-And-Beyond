...
func draw_triangle(center := Vector2.ZERO, angle := 0.0, radius := 10.0, in_color := DEFAULT_COLOR) -> void:
	var points := PackedVector2Array()
	var triangle_colors := PackedColorArray([in_color])
	for i in range(3):
		var angle_point := angle + i * 2.0 * PI / 3.0 + PI
		var offset := Vector2(radius * cos(angle_point), radius * sin(angle_point))
		var point := center + offset
		points.append(point)
	draw_polygon(points, triangle_colors)


func draw_circle_outline(
	circle_position := Vector2.ZERO, radius := 30.0, in_color := Color(), thickness := 1.0
) -> void:
	var points_array := PackedVector2Array()
	for i in range(CIRCLE_VERTEX_COUNT + 1):
		var angle := 2 * PI * i / CIRCLE_VERTEX_COUNT
		var point := circle_position + Vector2(cos(angle) * radius, sin(angle) * radius)
		points_array.append(point)
	draw_polyline(points_array, in_color, thickness, true)
